...
| `STAGE_TIMEOUT_MS_CODER` | no | 1800000 (30 min) | Per revision iteration; sizes the revision-loop default. Also floors `STAGE_TIMEOUT_MS_FIX_FINDINGS` (the round 2+ producer budget) and the in-loop verify gate's per-round fix budget when those vars are unset — it keeps mattering after round 1 |
...
| `STAGE_TIMEOUT_MS_REVISION_LOOP` | no | `MAX_REVISIONS × (PLAN? + max(CODER, FIX_FINDINGS) + IN-LOOP VERIFY + REVIEWER)` (135 min without a plan step or in-loop verify) | Wall-clock cap on the whole loop, now including the in-loop `verify` gate every round pays for (`(MAX_INLOOP_FIX_ATTEMPTS+1) × VERIFY_PASS + MAX_INLOOP_FIX_ATTEMPTS × CODER`, 0 when `SKIP_BUILD_TEST=true`). **Throughput note:** with the in-loop gate on, the derived default is `3 × (30 + 60 + 15)` = **315 min**, up from 135 — and at the default `CONCURRENCY=1` a single wedged work item can hold the only pipeline slot for over five hours before this timeout fires. Pin a smaller value here (or raise `CONCURRENCY`) if queue latency matters more than letting a slow loop finish |
...
| `MAX_REVISIONS` | no | 3 | Max revision-loop iterations per WI (round 1 coder, rounds 2+ fix-findings, each followed by verify + reviewer) |
